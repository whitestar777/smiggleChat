...
new Vue({
    ...
    events: {
        'open-messanger': function openMessanger(userToOpen) {
            this.openUser = userToOpen;
            $('#contactsSection').removeClass('large-12').addClass('large-8');
            this.$broadcast('open-for-messages');
        },
        'opened-messanger': function openedMessanger() {
            this.clearPending();
        },
        'message-send': function messageSend() {
            this.fetchUsers();

            //allUsers:[]
            //this.openUser.allMessages.push(message);
        },
        'close-messanger': function closeMessanger() {
    ...
    methods: {
        ...
        sendSeen: function sendSeen(message) {
            this.$http.post('/api/message/seen', message).then(function (response) {}.bind(this)).catch(function (response) {
                this.errors = response.data;
            }.bind(this));
        },
        updateOpenUser: function updateOpenUser() {
            //if(this.openUser.id){
            this.allUsers.forEach(function (user) {
...
},{"./components/Contacts/Contacts":2,"./components/Messages/Messanger":4,"./vue":8,"./vue-resource":6,"./vue-router":7}],2:[function(require,module,exports){
'use strict';
...
module.exports = {
    ...
    methods: {
        openMessager: function openMessager(user) {
            this.$dispatch('open-messanger', user);
        }
    }
};

},{"./views/contacts.html":3}],3:[function(require,module,exports){
module.exports = '    <div id="contacts" class="large-3 medium-4 small-6 column" v-for="user in allUsers">\n        <div class="contactWrapper">\n            <a v-on:cLick="openMessager(user)">{{user.email}}</a> <span class="messageCount" v-show="user.pending_messages.length !== 0">{{user.pending_messages.length}}</span>\n            <br>\n            <a v-on:cLick="openMessager(user)" class="button" v-show="user.allMessages.length === 0">chat</a>\n            <a v-on:cLick="openMessager(user)" class="button success" v-show="user.allMessages.length >= 1">continue</a>\n        </div>\n    </div>\n';
...
module.exports = {
    ...
    events: {
        'open-for-messages': function openForMessages() {
    ...
    methods: {
        ...
        closeMessanger: function closeMessanger() {
            this.$dispatch('close-messanger');
        }
    }
};

},{"./views/messanger.html":5}],5:[function(require,module,exports){
module.exports = '<div id="messanger" class="large-4 columns hide">\n    <div class="large-12 columns">\n        <a class="red" @click="closeMessanger">x</a>\n    </div>\n    <div class="large-6 columns">\n        {{openUser.email}}\n    </div>\n    <div class="large-6 columns text-right">\n        me\n    </div>\n    <hr>\n    <div class="large-12 columns"\n         v-for="message in openUser.allMessages | orderBy \'id\'">\n        <div class=" message" :class="[message.color]">{{message.content}}</div>\n    </div>\n    <div class="large-12 columns">\n        <div class="input-group-field">\n            <input type="text" id="newMessage" name="newMessage" class="input-group-field"\n                   v-model="newMessage">\n        </div>\n        <div class="input-group-button">\n            <button class="button" @click="sendMessage">send</button>\n        </div>\n    </div>\n</div>';
...
(function webpackUniversalModuleDefinition(root, factory) {
	if ((typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === 'object' && (typeof module === 'undefined' ? 'undefined' : _typeof(module)) === 'object') module.exports = factory();else if (typeof define === 'function' && define.amd) define([], factory);else if ((typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === 'object') exports["VueResource"] = factory();else root["VueResource"] = factory();
})(undefined, function () {
    ...
    */
			...
			module.exports = {
				...
				request: function request(_request3) {
					...
					if (_request3.method == 'JSONP') {
						_request3.client = jsonpClient;
					}
    ...
    */
			...
			module.exports = {
				...
				request: function request(_request4) {

					if (_request4.emulateHTTP && /^(PUT|PATCH|DELETE)$/i.test(_request4.method)) {
						_request4.headers['X-HTTP-Method-Override'] = _request4.method;
						_request4.method = 'POST';
					}
    ...
    */
			...
			module.exports = {
				...
				request: function request(_request5) {

					if (_request5.emulateJSON && _.isPlainObject(_request5.data)) {
						_request5.headers['Content-Type'] = 'application/x-www-form-urlencoded';
						_request5.data = _.url.params(_request5.data);
					}

					if (_.isObject(_request5.data) && /FormData/i.test(_request5.data.toString())) {
						delete _request5.headers['Content-Type'];
					}

					if (_.isPlainObject(_request5.data)) {
						_request5.data = JSON.stringify(_request5.data);
					}
				...
				response: function response(_response2) {
					...
					try {
						_response2.data = JSON.parse(_response2.data);
					} catch (e) {}
    ...
    */
			...
			module.exports = {
				...
				request: function request(_request6) {

					_request6.method = _request6.method.toUpperCase();
					_request6.headers = _.extend({}, _.http.headers.common, !_request6.crossOrigin ? _.http.headers.custom : {}, _.http.headers[_request6.method.toLowerCase()], _request6.headers);

					if (_.isPlainObject(_request6.data) && /^(GET|JSONP)$/i.test(_request6.method)) {
						_.extend(_request6.params, _request6.data);
						delete _request6.data;
					}
    ...
    */
			...
			module.exports = {
				...
				request: function request(_request7) {

					if (_request7.crossOrigin === null) {
						_request7.crossOrigin = crossOrigin(_request7);
					}

					if (_request7.crossOrigin) {

						if (!xhrCors) {
							_request7.client = xdrClient;
						}

						_request7.emulateHTTP = false;
					}
...
if(path.charAt(0)==='0'&&isNaN(path)){return false;}return isLiteral(trimmed)?stripQuotes(trimmed):'*'+trimmed;} /**
   ...
   */var warnNonExistent;if('development'!=='production'){warnNonExistent=function warnNonExistent(path,vm){warn('You are setting a non-existent path "'+path.raw+'" '+'on a vm instance. Consider pre-initializing the property '+'with the "data" option for more reliable reactivity '+'and better performance.',vm);};} /**
...
if('development'!=='production'&&has[id]!=null){circular[id]=(circular[id]||0)+1;if(circular[id]>config._maxUpdateCount){warn('You may have an infinite update loop for watcher '+'with expression "'+watcher.expression+'"',watcher.vm);break;}}}} /**
...
/* istanbul ignore if */if(!prevEl.nextSibling){this.end.parentNode.appendChild(this.end);}frag.before(prevEl.nextSibling,false);}, /**
     ...
     * @param {Number} index
     * @param {String} [key]
     */cacheFrag:function cacheFrag(value,frag,index,key){var trackByKey=this.params.trackBy;var cache=this.cache;var primitive=!isObject(value);var id;if(key||trackByKey||primitive){id=trackByKey?trackByKey==='$index'?index:getPath(value,trackByKey):key||value;if(!cache[id]){cache[id]=frag;}else if(trackByKey!=='$index'){'development'!=='production'&&this.warnDuplicate(value);}}else {id=this.id;if(hasOwn(value,id)){if(value[id]===null){value[id]=frag;}else {'development'!=='production'&&this.warnDuplicate(value);}}else {def(value,id,frag);}}frag.raw=value;}, /**
     ...
     * @param {String} key
     * @return {Fragment}
     */getCachedFrag:function getCachedFrag(value,index,key){var trackByKey=this.params.trackBy;var primitive=!isObject(value);var frag;if(key||trackByKey||primitive){var id=trackByKey?trackByKey==='$index'?index:getPath(value,trackByKey):key||value;frag=this.cache[id];}else {frag=value[this.id];}if(frag&&(frag.reused||frag.fresh)){'development'!=='production'&&this.warnDuplicate(value);}return frag;}, /**
...
if(value!=null)value+='';if(value){var isImportant=importantRE.test(value)?'important':'';if(isImportant){ /* istanbul ignore if */if('development'!=='production'){warn('It\'s probably a bad idea to use !important with inline rules. '+'This feature will be deprecated in a future version of Vue.');}value=value.replace(importantRE,'').trim();this.el.style.setProperty(prop.kebab,value,isImportant);}else {this.el.style[prop.camel]=value;}}else {this.el.style[prop.camel]='';}}}; /**
...
this.type=hooks&&hooks.type; /* istanbul ignore if */if('development'!=='production'){if(this.type&&this.type!==TYPE_TRANSITION&&this.type!==TYPE_ANIMATION){warn('invalid CSS transition type for transition="'+this.id+'": '+this.type,vm);}} // bind
...
if('development'!=='production'){if(name==='class'&&Array.prototype.some.call(attrs,function(attr){return attr.name===':class'||attr.name==='v-bind:class';})){warn('class="'+rawValue+'": Do not mix mustache interpolation '+'and v-bind for "class" on the same element. Use one or the other.',options);}}}else  // special attribute: transition
...
if(!props||!hasOwn(props,key)||runtimeData&&hasOwn(runtimeData,key)&&props[key].raw===null){this._proxy(key);}else if('development'!=='production'){warn('Data field "'+key+'" is already defined '+'as a prop. Use prop default value instead.',this);}} // observe data
observe(data,this);}; /**
